...
#' 
#' It is advisable to first check that a GPT model file is valid using \code{\link{read.gpt}}. Next, one can either first generate some simulated data using \code{\link{gen.gpt}} or fit data using \code{\link{fit.gpt}}. The fitting algorithm first uses an EM algorithm before maximizing the full likelihood by gradient descent. Note that restrictions on the parameter space are automatically taken into account (e.g., variances must be positive). 
#' 
